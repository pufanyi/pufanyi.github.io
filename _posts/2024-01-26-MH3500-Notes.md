...
接下来是 $$t$$ distribution。Motivation 是这样的，我们现在还是有 $$n$$ 个 i.i.d. $$\mathcal{N}(\mu, \sigma^2)$$ 的 random variables，$$X_1, X_2, \dots, X_n$$。我们知道 $$\frac{\left(\overline{X}-\mu\right)}{\sigma/\sqrt{n}}\sim \mathcal{N}(0, 1)$$ 我们现在想知道的是 $$Z=\frac{\overline{X}-\mu}{S/\sqrt{n}}$$ 的 distribution 长啥样。
