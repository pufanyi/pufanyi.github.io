...
有一个好玩的性质是 $$\overline{X}$$ 和 $$S^2$$ 是独立的。但因为水平原因不太能证。上课老师只讲了当 $$X_i\sim N(\mu, \sigma^2)$$ 的时候咋搞。然后他让我们去翻 Rice 的书，哪里讲的是当 $$\boldsymbol{X}$$ 是 multivariate normal distribution 的时候咋搞。

{% details 上课老师讲的证法 %}

其实我至今没搞懂他到底是咋搞的。因为他用了一个结论，就是两个协方差为 $$0$$ 的 normal distribution 是相互独立的。不是很清楚不用 mgf 这玩意儿还能咋证。
...
$$
M_{\boldsymbol{\widehat{X}}}\left(\boldsymbol{\widehat{t}}\right)=M_{\overline{X}}(s)\cdot M_{X_1-\overline{X}}(t_1)\cdot M_{X_2-\overline{X}}(t_2)\cdots M_{X_n-\overline{X}}(t_n)
$$
...
为啥要这样转化呢？因为虽然我们现在不知道 $$\widehat{\boldsymbol{X}}$$ 的独立性，但是 $$\boldsymbol{X}$$ 的精神状态咋样我们是知道的。
