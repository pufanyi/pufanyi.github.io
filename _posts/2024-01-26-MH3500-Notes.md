...
我们考虑另一个 estimator $$\hat{\theta}=\frac{X+1}{n+2}=w\frac{X}{n}+(1-w)\frac{1}{2}$$，其中 $$w=\frac{n}{n+2}$$。于是乎这个 estimator 的 variance 是
$$
\mathrm{Var}(\hat{\theta})=w^2\cdot\frac{\mathrm{Var}(X)}{n^2}=\left(\frac{n}{n+1}\right)^2\cdot\frac{\theta\cdot(1-\theta)}{n^2}
$$
{% enddetails %}
