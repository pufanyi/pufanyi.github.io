...
教材用的是这三本：<d-cite key="hogg2019introduction"></d-cite><d-cite key="rice2006mathematical"></d-cite><d-cite key=""></d-cite>
...
一开始其实我没想明白一件事情。考虑一个 joint moment generating function $$M_{X, Y}(s, t)=\mathbb{E}\left[e^{sX+tY}\right]$$，假设我们能推出 $$M_{X, Y}(s, t)=M_X(s)M_Y(t)$$，那么我们就能说明 $$X, Y$$ 独立的。

这个命题反过来是很显然的，但是正过来感觉有点神奇。但仔细思考发现其实就是 $$\mathbb{E}\left[e^{sX+tY}\right]=\mathbb{E}\left[e^{sX}\right]\cdot\mathbb{E}\left[e^{sY}\right]$$。也就是说 $$e^{sX}$$ 和 $$e^{tY}$$ 是独立的，那很显然 $$X, Y$$ 也是独立的了。

$$M_{\overline{X}, X_1-\overline{X}, \dots, X_n-\overline{X}}(s, t_1, \dots, t_n)$$，
