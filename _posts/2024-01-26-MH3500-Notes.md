...
- $$\mathcal{R}_5$$: The PDF $$f(x\theta\theta)$$ is three times differentiable as a function of $$\theta$$. Further, for
all $$\theta\in\Omega$$, there exist a constant $$c$$ and a function $$M(x)$$ such that $$\left|\frac{\partial^3}{\partial\theta^3}\log f(x\mid\theta)\right|\le M(x)$$, with $$\mathbb{E}_{\theta_0}[M(X)] < \infty$$, for all $$θ_0 - c < \theta < \theta_0 + c$$ and all $$x$$ in the support of $$X$$.
