...
&=\int_{-\infty}^{+\infty}|y|\cdot\frac{1}{\sqrt{2\pi}}e^{-\frac{(zy)^2}{2}}\cdot\frac{k^{\frac{k}{2}}{|y|}^{k-1}e^{-\frac{ky^2}{2}}}{2^{\frac{k}{2}-1}\Gamma\left(\frac{k}{2}\right)}\mathrm{d}y\\
&=\frac{k^{\frac{k}{2}}}{\sqrt{2\pi}\cdot 2^{\frac{k}{2}-1}\cdot \Gamma\left(\frac{k}{2}\right)}\int_{-\infty}^{+\infty}{|y|}^ke^{-\frac{(k+z^2)y^2}{2}}\mathrm{d}y\\
&=\frac{k^{\frac{k}{2}}}{\sqrt{2\pi}\cdot 2^{\frac{k}{2}}\cdot \Gamma\left(\frac{k}{2}\right)}\int_{0}^{+\infty}y^ke^{-\frac{(k+z^2)y^2}{2}}\mathrm{d}y
\end{aligned}
$$

我们发现这个后面那个积分式子特别像 $$\Gamma$$ 函数，于是我们来凑一凑。考虑到 $$\Gamma$$ 函数中 $$e$$ 那边是个 $$e^{-t}$$ 的形式，那么就换个元。为了方便书写我们假设 $$\alpha=\frac{k+z^2}{2}$$，$$t=\alpha y^2$$。

于是乎我们就有：

$$
\begin{aligned}
\int_{0}^{+\infty}y^ke^{-\frac{(k+z^2)y^2}{2}}\mathrm{d}y&=\int_{0}^{+\infty}\left(\frac{t}{\alpha}\right)^\frac{k}{2}e^{-t}\cdot \mathrm{d}\left(\sqrt{\frac{t}{\alpha}}\right)\\
&=\int_0^{+\infty}\left(\frac{t}{\alpha}\right)^{\frac{k}{2}}e^{-t}\cdot \frac{1}{\alpha}\cdot\frac{1}{2}\cdot\sqrt{\frac{\alpha}{t}}\cdot\mathrm{d}t\\
&=\frac{1}{2\alpha}\int_0^{+\infty}\left(\frac{t}{\alpha}\right)^\frac{k-1}{2}e^{-t}\mathrm{d}t\\
&=\frac{1}{2\alpha^{\frac{k+1}{2}}}\int_0^{+\infty}t^{\frac{k-1}{2}}e^{-t}\mathrm{d}t\\
&=\frac{1}{2\alpha^{\frac{k+1}{2}}}\int_0^{+\infty}t^{\left(\frac{k+1}{2}\right)-1}e^{-t}\mathrm{d}t\\
&=\frac{1}{2\alpha^{\frac{k+1}{2}}}\cdot\Gamma\left(\frac{k+1}{2}\right)\\
\end{aligned}
$$

然后我们最后把所有东西一股脑代进去得到答案：
