...
$$
-\frac{\frac{\partial}{\partial\theta}\ell(\theta)}{\frac{\partial^2}{\partial\theta^2}\ell(\theta)}=
-\frac{\sum_{i=1}^n\frac{\partial}{\partial\theta}\log f(x_i\mid\theta)}{\frac{\partial^2}{\partial\theta^2}\ell(\theta)}=\sum_{i=1}^n\frac{\frac{\partial}{\partial\theta}\log f(x_i\mid\theta)}{-\frac{\partial^2}{\partial\theta^2}\ell(\theta)}
$$

是一个和式的形式，根据 CLT，当 $$n$$ 足够大的时候，我们有：

$$
-\frac{\frac{\partial}{\partial\theta}\ell(\theta)}{\frac{\partial^2}{\partial\theta^2}\ell(\theta)}\to\mathcal{N}\left(\tilde{\mu}, \tilde{\sigma}^2\right)
$$
...
$$
\begin{aligned}
\tilde{\mu}&=\mathbb{E}\left[\frac{\partial}{\partial\theta}\ell(x\mid\theta)\right]\\
&=\int_{-\infty}^{\infty}\frac{\partial}{\partial\theta}\ell(x\mid\theta)\cdot f(x\mid\theta)\mathrm{d}x\\
&=\int_{-\infty}^{\infty}\frac{\partial}{\partial\theta}\log f(x\mid\theta)\cdot f(x\mid\theta)\mathrm{d}x\\
&=\int_{-\infty}^{\infty}\frac{\frac{\partial}{\partial\theta}f(x\mid\theta)}{f(x\mid\theta)}\cdot f(x\mid\theta)\mathrm{d}x\\
&=\int_{-\infty}^{\infty}\frac{\partial}{\partial\theta}f(x\mid\theta)\mathrm{d}x\\
&=\frac{\partial}{\partial\theta}\int_{-\infty}^{\infty}f(x\mid\theta)\mathrm{d}x\\
&=\frac{\partial}{\partial\theta}1=0
\end{aligned}
$$
...
$$
\frac{\partial}{\partial\theta}\ell(\theta)\to\mathcal{N}\left(0, \mathbb{E}\left[\left(\frac{\partial}{\partial\theta}\ell(\theta)\right)\right]\right)
$$
