...
$$
\mathcal{H}(p, q)=-\mathbb{E}_p\left[\log q\right]
$$
...
$$
\mathcal{H}(\mathcal{X})=-\mathbb{E}[\mathcal{S}(p_\mathcal{X})]=-\mathbb{E}[\log_2 p_\mathcal{X}]=\begin{cases}
\sum_{x\in\mathcal{X}}p(x)\log_2\frac{1}{p(x)} & \text{Discrete}\\
\int_{\mathcal{X}}p(x)\log_2\frac{1}{p(x)}\mathrm{d}x & \text{Continuous}
\end{cases}
$$
{% enddetails %}
