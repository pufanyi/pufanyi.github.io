...
**(a)**
$$
\exists x\in\R\ ((x^2=0)\land(\forall y\in\R\ ((y^2=0)\implies (x=y)))).
$$
...
1. $$\exists k\in [0, n-1]\cap\Z, \forall i, a_i\neq k$$. Which means, $$\forall i, a_i\in [0, n-1]\cap\Z\backslash\{k\}$$. And since $$\left|[0, n-1]\cap\Z\backslash\{k\}\right|=n-1$$, and there are $$n$$ numbers. Due to the Pigeonhole Principle, there must be $$2$$ of them have the same number.
2. If $$\forall k\in [0, n-1]\cap\Z, \exists i, a_i=k$$, there must be some one with $$0$$ friends (name A) and some one with $$n-1$$ friends (name B). A have $$0$$ friends shows that A and B are not friends, but B have $$n-1$$ friends shows that A and B are friends. This forms a contradiction, so this case is impossible.
