...
3. **Security is economics**：搞 security 需要钱，所以要考虑你保护的是啥。
...
7. **Ensure complete mediation**：You should check _every_ access to _every_ object. 课上提出了一个叫做 **Reference Monitor** 的概念，就是一个任何 access 操作都必须经过的结点，比如说 network firewall。用这种方法来保证每个 access 都被检查过。
