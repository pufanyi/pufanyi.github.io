...
&=\sum_{n=0}^\infty\frac{(2n)!}{(n!)}\left(pqs^2\right)^n\\
...
然后我们来计算 $$\mathbb{E}\left[T_0^r\mid S_0=0\right]$$ 的时候会发现如果 $$\mathbb{P}\left(T_0^r\mid S_0=0\right)>0$$ 的时候这个期望肯定是 $$\infty$$。而 $$\mathbb{P}\left(T_0^r\mid S_0=0\right)$$ 只有在 $$p=q=\frac{1}{2}$$ 的时候才会为 $$0$$。
